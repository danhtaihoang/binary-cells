...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%! 
      ...
      CHARACTER (LEN=3)  :: SAt
      CHARACTER (256)    :: Ligne21,Ligne22,Ligne23,Ligne24
      ...
      REAL    (KIND=8) :: Jabmin,Jabmax,del_Jab,FNab_times_av,Nab_times,Nab2_times,FNab_times
      REAL    (KIND=8) :: NaaNbb_av,FPab_indirect
...
!!!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      OPEN(unit=24,file='FPab_times.dat')
...
!!!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      im=tab_im(i)
           ...
           DO WHILE ((int(Sa(inn,jnn,knn)+Sb(inn,jnn,knn))==0)&
                  .or.(int(Sa(inn,jnn,knn))==int(Sa(i,j,k))))
           ...
           !IF (int(Sa(inn,jnn,knn))==int(Sa(i,j,k))) THEN
...
!!==========================================================================================                       
           ...
           energy=energy+E1_old
            ...
            !Naa=Naa+Sa(i,j,k)*(Sa(im,j,k)+Sa(ip,j,k)) 
            !Nbb=Nbb+Sb(i,j,k)*(Sb(im,j,k)+Sb(ip,j,k))              
...
!!!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      Naa2_av=0.; Nbb2_av=0. ; Nab2_av=0. ; NaaNbb_av=0.

      FNab_times_av=0.

      DO i_times=1,n_times

            Nab_times=0. ; Nab2_times=0. ; FNab_times=0.
            ...
            DO i_loop2=1,n_average
                  ...
                  Nab_times=Nab_times+Nab; Nab2_times=Nab2_times+Nab2

                  NaaNbb_av=NaaNbb_av+Naa*Nbb
            ...
            Nab_times=Nab_times/real(n_average)
            Nab2_times=Nab2_times/real(n_average)
            FNab_times=Nab2_times-Nab_times**2.

            FNab_times_av=FNab_times_av+FNab_times

      END DO

      FNab_times_av=FNab_times_av/real(n_times*(natx-1))
      ...
      !!!---------------------
      NaaNbb_av=NaaNbb_av/real(n_times*n_average)
      FPab_indirect=FPaa+FPbb+2.*NaaNbb_av/N_total/N_total-2.*Paa*Pbb
      ...
      WRITE(Ligne24,*) Jab,FNab_times_av,FPab_indirect
